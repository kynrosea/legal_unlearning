...
# loading subset of contracts for constructing forget and retain sets
contracts_data = du.load_contracts_data(DATASET, SUBSET, 'train', SAMPLE_SIZE, SEED)

# extracting and validating names from contracts dataset
...
# building forget and retain sets
...
# computing evaluation metrics before NPO training
...
# attaching LoRA adapters and creating a frozen reference model copy
model = mu.attach_lora(model)
ref_model = mu.make_reference_model(model)

# running NPO training loop
...
# computing evaluation metrics after NPO training
